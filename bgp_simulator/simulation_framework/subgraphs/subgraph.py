...
ofile = open(
    '/home/zhao/Shujie/Routing_traffic/coding/bgpsimulator/stealthy_hijacks_res/degree_centrality_as_paths.res', 'w')
...
class Subgraph(ABC):
    ...
    def _get_engine_outcomes(self,
        ...
        for as_obj in engine.as_dict.values():
            ...
            self._get_as_outcome_rovpp(as_obj, engine, scenario)
    ...
    def _get_as_outcome_v2(self, as_obj: AS, engine: SimulationEngine, scenario: Scenario):
        ...
        #    print("victim's BGP neighbors: ", as_obj.asn, as_obj.__to_yaml_dict__()['providers'], as_obj.__to_yaml_dict__()['customers'], as_obj.__to_yaml_dict__()['peers'])
        ...
        if len(most_specific_anns) > 0 and (as_obj.asn not in scenario.attacker_asns) and (as_obj.asn not in scenario.victim_asns):
            ...
            for most_specific_ann in most_specific_anns:
                ...
                ofile.write(','.join(map(str, list(as_path)))+'\n')
                ...
                if prefix == Prefixes.SUBPREFIX.value:
                    ...
                    #print('as_path: ', as_path, as_path[1])
    ...
    def _get_as_outcome_rovpp(self, as_obj: AS, engine: SimulationEngine, scenario: Scenario):
        ...
        sub_ann = as_obj._local_rib.get_ann(Prefixes.SUBPREFIX.value)
        if sub_ann:
        	if not sub_ann.blackhole:
        		scenario.affected_prefixes[Prefixes.SUBPREFIX.value].add(as_obj.asn) # directly impact
        	else:
        		scenario.affected_prefixes[Prefixes.PREFIX.value].add(as_obj.asn) # indirectly impact
        else: # only victim prefix or no ann
        	ann = as_obj._local_rib.get_ann(Prefixes.PREFIX.value)
        	if ann:
        		prefix = ann.prefix
        		as_path = ann.as_path
        		for node in as_path[1:-1]:
        			if node in scenario.affected_asns:
        				...
        				node_as_obj = self.convert_to_as_obj(node, engine)
        				if node_as_obj != None:
        					if self._get_as_outcome_rovpp(node_as_obj, engine, scenario):
        						affected = True
        						scenario.affected_prefixes[Prefixes.PREFIX.value].add(as_obj.asn) # indirectly impact
        						break
    ...
    def _get_as_outcome(self,
        ...
        input_clique = {174, 209, 286, 701, 1239, 1299, 2828, 2914, 3257, 3320, 3356, 3491, 5511, 6453, 6461, 6762, 6830, 7018, 12956}
        ...
        if as_obj in outcomes:
            ...
            if len(most_specific_anns) > 0:
                ...
                for most_specific_ann in most_specific_anns:
                    ...
                    lens.append(L)
                    for asn in most_specific_ann.as_path:
                    	if asn in input_clique:
                    		tier1 = True
                    asID = int(most_specific_ann.origin)
                    vrpID = int(most_specific_ann.roa_origin)
                    n = n + 1
                    ...
                    if as_obj.asn == 16071: print(most_specific_ann)

                ofile.write(str(as_obj.asn)+','+str(min(lens))+','+str(tier1)+'\n')
